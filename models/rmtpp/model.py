...
class RMTPP(nn.Module):
    def __init__(self, config) -> None:
        ...
        self.init_hidden = nn.Parameter(torch.randn(hidden_dim))
        self.register_buffer('device_indicator', torch.empty(0))
        self.dtime_w = nn.Parameter(torch.randn(1))
        self.dtime_b = nn.Parameter(torch.randn(1))

    def compute_loss(self, embed_seq, type_seq, time_seq):
        device = self.device_indicator.device
        dtimes = (time_seq[:, 1:] - time_seq[:, :-1]).to(device).unsqueeze(-1)
        embed_seq = embed_seq[:, 1:, :].to(device)
        ...
        time_log_prob = time_enc + self.dtime_w * dtimes + self.dtime_b +\
         (torch.exp(time_enc+self.dtime_b)-torch.exp(time_enc + self.dtime_w * dtimes + self.dtime_b))\
              / self.dtime_w
